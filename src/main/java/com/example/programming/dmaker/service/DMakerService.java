...
public class DMakerService {
  ...
  public CreateDeveloper.Response createDeveloper(CreateDeveloper.Request request) {
    ...
    Developer developer = Developer.builder().developerLevel(request.getDeveloperLevel())
        .developerSkillType(request.getDeveloperSkillType())
        .experienceYears(request.getExperienceYears()).name(request.getName())
        .statusCode(StatusCode.EMPLOYED).age(request.getAge()).memberId(request.getMemberId())
        .build();
  ...
  private void validateDeveloperLevel(DeveloperLevel developerLevel, Integer experienYears) {
    if (developerLevel == DeveloperLevel.SENIOR && experienYears < 10) {
      throw new DMakerException(LEVEL_EXPERIENCE_YEARS_NOT_MATCHED);
    }
    if (developerLevel == DeveloperLevel.JUNGNIOR && (experienYears < 4 || experienYears > 10)) {
      throw new DMakerException(LEVEL_EXPERIENCE_YEARS_NOT_MATCHED);
    }
  ...
  public DeveloperDetailDto deleteDeveloper(String memberId) {
    ...
    /*
     * 아래 if문 처럼 작성하면 트랜잭션을 하다가 예외가 걸려서 RetiredDeveloper을 저장하지 못하면 @Transactional 어노테이션이 롤백을 지원해준다.
     * 하지만 @Transactional 설정을 안해놓는다면 Developer table 에서는 트랜잭션이 성공하고 RetiredDeveloper table 에서는 저장이
     * 안되는 롤백이 안되는 일이 일어난다..... Atomic이 깨져버리는거다. 꼭 하나의 트랜잭션이라도 @Transactional 을 사용하자.
     * 
    ...
    RetiredDeveloper retiredDeveloper =
        RetiredDeveloper.builder().memberId(memberId).name(developer.getName()).build();
